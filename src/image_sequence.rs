...
impl ImageSequence {
	...
	pub fn load(
		&mut self,
		file_cache: &mut std::sync::Arc<std::sync::Mutex<FileCache>>,
	) -> anyhow::Result<()> {
		if self.entries.len() == 0 {
			// :HACK: to allow multiple calls for now
			if self.filename != "" {
	...
	fn add_image(&mut self, file_cache: &mut FileCache, filename: &str) -> anyhow::Result<()> {
		...
		if data.len() > 0 {
			// only true if cache blocks on initial load, or somebody else already requested the same file earlier
